...
- Departments **Controladoria + Comercial + Financeiro** merge into one page, label `"Centro de Serviços Compartilhados"`, slug `centro-servicos-compartilhados`.
...
- "Config/isento" (não elegível) threshold: débito do período trabalhado ≥ 300h (18000 min) **and** crédito do período trabalhado ≤ 1h (60 min) — verified against the real xlsx, yields exactly the 9 flagged records the reference artifact describes.
